...
function plot_model_stats(stats_df, model_name)
    ...
    p = plot(layout=(1,4), size=(2000,300), top_margin=3Plots.mm, left_margin=5Plots.mm,right_margin=5Plots.mm, dpi=300, margin=1*Plots.mm, bottom_margin=2Plots.mm)
    ...
    # Plot FHV histogram
    histogram!(p[3], rmse_test, bins=0:0.5:10, label="RMSE", color=palette1[3], tickfontsize=12, legendfontsize=12, fontfamily="Times")
    ...
    annotate!(p[3], mean(xlims(p[3])), maximum(ylims(p[3]))*1.05, "mean=$(round(rmse_mean,digits=3)), median=$(round(rmse_median,digits=3))", fontsize=12, fontfamily="Times")

    # Plot FHV histogram
    histogram!(p[4], fhv_test, bins=0:2:75, label="FHV", color=palette1[4], tickfontsize=12, legendfontsize=12, fontfamily="Times")
    ...
    annotate!(p[4], mean(xlims(p[4])), maximum(ylims(p[4]))*1.05, "mean=$(round(fhv_mean,digits=3)), median=$(round(fhv_median,digits=3))", fontsize=12, fontfamily="Times")
