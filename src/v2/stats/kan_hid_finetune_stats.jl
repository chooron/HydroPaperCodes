...
for hidd_dims in [4, 5, 6, 7, 8]
    ...
    for basin_id in basins_available
        open("result/finetune/kan(q)_$(hidd_dims)_6/$(basin_id).json", "r") do f
            data = JSON.parse(f)
            push!(json_list, data)
        end
    end
...
for (hidd_dims, df) in zip([4, 5, 6, 7, 8], df_list)
    ...
    savefig(p, "result/finetune/plots/kan(q)_$(hidd_dims)_6_mse_dist.png")
end
...
CSV.write("result/finetune/kan(q)_hidd_stats.csv", stat_df)
