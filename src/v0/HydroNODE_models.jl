...
function basic_bucket_incl_states(p_, t_out)
    ...
    prob = DiscreteProblem(exp_hydro_optim_states!, p_[1:2], Float64.((t_out[1], maximum(t_out))))

    sol = solve(prob, FunctionMap(scale_by_time=true), u0 = p_[1:2], p=p_[3:end], saveat=t_out)
...
function NeuralODE_M50(p, t_out, ann, p_bucket_precal, addfeature; S_init = [0.0, 0.0])
    ...
    prob = DiscreteProblem(NeuralODE_M50_core!, S_init, Float64.((t_out[1], maximum(t_out))), p)

    sol = solve(prob, FunctionMap(scale_by_time=true), dt=1.0, saveat=t_out)
...
function NeuralODE_M100(p, t_out, ann, args...; S_init = [0.0, 0.0])
    ...
    sol = solve(prob, BS3(), dt=1.0, saveat=t_out, reltol=1e-3, abstol=1e-3, sensealg=GaussAdjoint(autojacvec=EnzymeVJP()))
