...
struct KAN{D,M} <: LuxCore.AbstractLuxLayer
    ...
    function KAN(width::Vector{Int}, mult_arity::Vector{<:Dict}, layer_cfg::Vector{<:Dict}, layer_type)
        ...
        new{layer_type,exist_mult}(in_dims, out_dims, NamedTuple(zip(layer_names, layers)), mult_ids)
    end
end
...
@generated function applykan(layers::NamedTuple{fields}, mult_ids::Vector{Vector}, x::AbstractArray{T,2}, ps, st) where {T,fields}
    N = length(fields)
