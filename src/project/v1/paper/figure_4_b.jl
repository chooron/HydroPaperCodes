...
original_et_nn = build_ET_NN(6, 6)
...
for basin_id in basin_ids
    ...
    layer_params = kan_ckpt["et"]
    et_nn_input = load_nn_data(basin_id, "exphydro(disc,withst)", "et")
    splines_dict = obtain_nn_splines(model_layers=model_layers, layer_params=layer_params, input_data=et_nn_input)
...
for (i, min_postact_score) in enumerate(min_postact_scores)
    ...
    colors[min_postact_score .< 0.1] .= :gray
...
savefig("src/paper/figures/figure_4_b.png")
