...
function build_M50_NNs(; hidd_dims = 16)
	...
	psnn = Lux.Chain(
		...
		Lux.Dense(hidd_dims, 2, sigmoid_fast),
	)
	return psnn
end

function get_default_params(; hidd_dims = 16, rng = StableRNG(42))
	ann_ps = build_M50_NNs(hidd_dims = hidd_dims)
...
function build_M50_P(itpfuncs, normfuncs;
	...
	function M50_ODE_core(S, p, t)
		...
		g_ps = psnn_func([norm_S2(S2), norm_S3(S3)], view(p, :ann_ps))
		# unnormalize
	...
	function M50_P_model(input, params, timesteps)
		TT, CFMAX, CFR, CWH, LP, FC, _, PPERC, UZL, k0, k1, k2 = params[:hbv_ps]
