...
include("models/exphydro_pro.jl")
...
model_name = "exphydro_pro_2(disc)"
...
function main(basin_id)
	...
	if model_name == "exphydro_pro_2(disc)"
		exphydro_model = build_exphydro_pro_2([itp_Lday, itp_P, itp_T], solve_type = "discrete")
		# f, Smax, Qmax, Df, Tmax, Tmin, f1, fp, S0, S1
		lower_bounds = [0.0, 100.0, 10.0, 0.01, 0.0, -3.0, 0.01, 0.01, 0.01, 100.0]
		upper_bounds = [0.1, 1500.0, 50.0, 5.0, 3.0, 0.0, 0.99, 0.99, 1500.0, 1500.0]
	else
		exphydro_model = build_exphydro_pro_1([itp_Lday, itp_P, itp_T], solve_type = "discrete")
		# f, Smax, Df, Tmax, Tmin, f1, fp, S0, S1
		lower_bounds = [0.0, 100.0, 0.01, 0.0, -3.0, 0.01, 0.01, 0.01, 100.0]
		upper_bounds = [0.1, 1500.0, 5.0, 3.0, 0.0, 0.99, 0.99, 1500.0, 1500.0]
	end

	default_ps_vec = lower_bounds .+ 0.5 * (upper_bounds.-lower_bounds)
	loss_func = (y, y_hat) -> sum((y .- y_hat) .^ 2) / sum((y .- mean(y)) .^ 2)
	opt_params, opt_recorder = calibrate_exphydro_pro2(
	...
	output = exphydro_model(total_arr, opt_params[1:8], opt_params[9:10], data_timepoints, return_all = true)
