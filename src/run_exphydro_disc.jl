...
model_name = "exphydro(cont2,withst)" # 离散模型,考虑初始状态
...
function main(basin_id)
	...
	exphydro_model = build_exphydro([itp_Lday, itp_P, itp_T], solve_type = "continous")
	lower_bounds = [0.0, 100.0, 10.0, 0.01, 0.0, -3.0, 0.01, 100.0]
	upper_bounds = [0.1, 1500.0, 50.0, 5.0, 3.0, 0.0, 1500.0, 1500.0]
	...
	output = exphydro_model(total_arr, opt_params[1:6], opt_params[7:8], data_timepoints, return_all = true)
